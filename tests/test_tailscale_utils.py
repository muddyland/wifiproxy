...
class TestGetStatus:
    def test_parses_connected_status(self, app):
        with app.app_context():
            with patch("app.tailscale.utils.is_installed", return_value=True), \
                 patch("app.tailscale.utils._run", return_value=_completed(SAMPLE_STATUS_JSON)):
    ...
    def test_parse_error_returns_safe_defaults(self, app):
        with app.app_context():
            with patch("app.tailscale.utils.is_installed", return_value=True), \
                 patch("app.tailscale.utils._run", return_value=_completed("bad json")):
                status = utils.get_status()
        assert status["connected"] is False
    ...
    def test_disconnected_state(self, app):
        ...
        with app.app_context():
            with patch("app.tailscale.utils.is_installed", return_value=True), \
                 patch("app.tailscale.utils._run", return_value=_completed(disconnected)):
                status = utils.get_status()
        assert status["connected"] is False
